...
export default function TaskEditPopup({ isOpen, onClose, onSave, categories, task, isUpdating = false }: TaskEditPopupProps) {
  ...
  const validateForm = () => {
    ...
    if (!description.trim()) {
      newErrors.description = t('popups:task_edit_popup.validation.descriptionRequired')
    }
    if (!dueDate) {
      newErrors.dueDate = t('popups:task_edit_popup.validation.dueDateRequired')
    }
    if (!dueTime) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      ...
      addNotification('error', t('popups:task_edit_popup.validation.errorTitle'), t('popups:task_edit_popup.validation.errorMessage'))
      return
    }
        ...
        >
          ...
          >
            ...
            <div className={`relative ${isDarkTheme ? 'bg-gray-900 bg-opacity-80 p-6 text-white' : 'bg-white bg-opacity-60 p-6 backdrop-blur-sm text-gray-800'}`}>
              ...
              <h2 className={`${isDarkTheme ? 'text-white' : 'text-gray-800'} text-2xl font-bold mb-6`}>{t('popups:task_edit_popup.title')}</h2>

              <form id="task-form" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1 sm:col-span-2">
                  <label className={`${isDarkTheme ? 'text-gray-300' : 'text-gray-700'} block text-sm font-medium`}>{t('popups:task_edit_popup.labels.title')}</label>
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className={`${isDarkTheme ? 'text-gray-300' : 'text-gray-700'} block text-sm font-medium`}>{t('popups:task_edit_popup.labels.description')}</label>
                ...
                <div className="space-y-1">
                  <label className={`${isDarkTheme ? 'text-gray-300' : 'text-gray-700'} block text-sm font-medium`}>{t('popups:task_edit_popup.labels.category')}</label>
                  <div className="flex items-center space-x-2">
                ...
                <div className="space-y-1">
                  <label className={`${isDarkTheme ? 'text-gray-300' : 'text-gray-700'} block text-sm font-medium`}>{t('popups:task_edit_popup.labels.priority')}</label>
                  <div className="flex items-center space-x-2">
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className={`${isDarkTheme ? 'text-gray-300' : 'text-gray-700'} block text-sm font-medium`}>{t('popups:task_edit_popup.labels.dueDateTime')}</label>
                  <div className="flex items-center space-x-2">

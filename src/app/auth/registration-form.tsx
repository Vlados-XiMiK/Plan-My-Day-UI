...
export default function RegistrationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email && password && confirmPassword) {
      ...
      setPendingNotification('success', 'Registration Successful. Welcome!', 5000)
      
      // Redirect immediately
      router.push('/')
    } else {
      addNotification('error', 'It looks like some fields are missing. Please complete all fields to continue..')
    }
  }

  return (
    <div className="w-full flex flex-col items-center">
      <h2 className="text-3xl font-bold text-blue-800 dark:text-violet-200 mb-6 text-center">New User Registration</h2>
      <form onSubmit={handleSubmit} className="space-y-6 w-full max-w-md">
        <div className="space-y-2">
          <label className="text-blue-800 dark:text-violet-200 text-sm font-medium">Email</label>
        ...
        <div className="space-y-2">
          <label className="text-blue-800 dark:text-violet-200 text-sm font-medium">Password</label>
          <div className="relative group">
        ...
        <div className="space-y-2">
          <label className="text-blue-800 dark:text-violet-200 text-sm font-medium">Confirm Password</label>
          <div className="relative group">
            <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-blue-600 dark:text-blue-300 group-hover:text-blue-800 dark:group-hover:text-white transition-colors duration-200" />
            <Input
